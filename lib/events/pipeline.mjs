...
function buildCandidateSummaries(groups, articleById, independenceGroupBySlug) {
  return groups.map((group) => {
    ...
    const confidenceState = group.overbroadCluster
      ? "REVIEW"
      : independentGroups < 2
        ? "REJECTED"
        : rawConfidenceState;
    const publishable = group.publishEligible && (confidenceState === "HIGH" || confidenceState === "LOW");
    ...
    for (const [index, articleId] of candidate.articleIds.entries()) {
      await prisma.eventMembership.create({
        data: {
          ...
          evidenceSummary: `independent groups: ${candidate.independentGroups}`,
        },
      });
    }
